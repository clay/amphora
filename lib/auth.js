...
function createSlackStrategy(site) {
  ...
  function (req, token, tokenSecret, profile, done) { // eslint-disable-line
    var uid = site.prefix + '/users/' + users.encode(`${profile._json.user.toLowerCase()}`, 'slack');
    ...
    if (!req.user) {
      ...
      return db.get(uid)
        ...
        .then(function (data) {
          return _.assign(data, {
            imageUrl: _.get(profile, '_json.info.user.profile.image_1024'),
            name: _.get(profile, '_json.info.user.real_name')
          });
        })
        .then(function (data) {
          return db.put(uid, JSON.stringify(data))
...
function addAuthRoutes(router, site) {
  return function (provider) {
    ...
    router.get(`/auth/${provider}/callback`, passport.authenticate(`${provider}-${site.slug}`, {
      failureRedirect: `${site.path}/auth/login`,
      failureFlash: true,
...
function init(router, providers, site) {
  ...
  if (providers.length) {
    ...
    router.get('/auth/login', (req, res) => res.send(tpl({ path: site.path, flash: req.flash() })));
