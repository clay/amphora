  ...
  return function (provider) {
    ...
    if (provider === 'google') {
      ...
      router.get(`/auth/${provider}`,
        ...
        }, passport.authenticate(`${provider}-${site.slug}`, {
          // passport options
          failureRedirect: `${site.path}/auth/login`,
          failureFlash: true,
          successReturnToOrRedirect: site.path }));
    } else {
...
function init(router, providers, site) {
  ...
  if (providers.length) {
    ...
    router.get('/auth/login', (req, res) => res.send(tpl({
      path: site.path,
      flash: req.flash(),
