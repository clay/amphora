...
function withAuthLevel(requiredLevel) {
  return function (req, res, next) {
    ...
    // We need to check if we're dealing with a CLAY_ACCESS_KEY user.
    if (!userLevel) {
      let apiKeyUser,
        authHeader = req.get('Authorization');


      apiCallback(authHeader && authHeader.split(' ')[1], (ctx, user) => {
        apiKeyUser = user;
      });

      if (apiKeyUser) {
        userLevel = apiKeyUser.auth;
      }
    }
