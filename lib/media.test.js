...
describe(_.startCase(filename), function () {
  var sandbox,
    ...
    componentScriptHtml = '<html><head></head><body>\n<!-- Scripts Begin -->\n<script type="text/javascript" src="/path/js/a.js"></script>\n<!-- Scripts End -->\n</body></html>',
  ...
  describe('appendTop', function () {
    ...
    it('adds nothing to bottom of head when no components', function () {
      ...
      expect(fn({components:[], site:site})(basicHtml)).to.deep.equal(basicHtml);
    });

    it('adds nothing to top of root when no components', function () {
      components.getStyles.onCall(0).returns([]);

      expect(fn({components:[], site:site})(basicSection)).to.deep.equal(basicSection);
    });

    it('adds to bottom of head', function () {
      components.getStyles.onCall(0).returns(['/css/a.css']);

      expect(fn({components:['a'], site:site})(basicHtml)).to.deep.equal(componentStyleHtml);
    });

    it('adds to top of root', function () {
      components.getStyles.onCall(0).returns(['/css/a.css']);

      expect(fn({components:['a'], site:site})(basicSection)).to.deep.equal(componentStyleSection);
    });
  });
  ...
  describe('appendBottom', function () {
    ...
    it('adds to bottom of body', function () {
      ...
      expect(fn({components:['a'], site:site})(basicHtml)).to.deep.equal(componentScriptHtml);
    });

    it('adds to bottom of root', function () {
      components.getScripts.onCall(0).returns(['/js/a.js']);

      expect(fn({components:['a'], site:site})(basicSection)).to.deep.equal(componentScriptSection);
    });
  });
});
