...
function renderPage(uri, req, res, hrStart) {
  ...
  // Do not allow users to be in edit mode for @published instances
  if (clayUtils.isPublished(uri) && req.query.edit) return res.redirect(locals.url.replace('@published', ''));
