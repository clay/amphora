...
const _ = require('lodash'),
  ...
  scopeUtil = require('./services/scopes'),
 ...
 * @return {Promise}
 */
function renderComponent(req, res, options, hrStart) {
  ...
  return components.get(uri, locals)
    ...
    .tap(logTime(hrStart, req.uri))
    .then(state => renderer.render(state, res));
}
...
function formDataForRenderer(options, locals) {
  return scopeUtil.resolveScopes(options.scopes, locals)
    .flatMap((queryResults) =>
      h(composer.resolveComponentReferences(options.data, locals, queryResults)))
    .map(applyOptions(options, locals))
    .toPromise(bluebird);
}
...
module.exports.resolveEnvVars = _.memoize(resolveEnvVars);
