...
function renderPage(uri, req, res) {
  ...
  return getDBObject(uri)
    ...
    .catch(e => {
      log('error', `Error rendering ${uri}: ${e.message}`, {
        action: 'renderPage'
      });
    });
}
...
function renderUri(uri, req, res) {
  return db.get(uri).then(function (result) {
    ...
    if (!route) {
      log('error', new Error(`Invalid URI: ${uri}: ${result}`));
    }
...
function renderExpressRoute(req, res, next) {
  ...
  return module.exports.renderUri(pageReference, req, res).then((resp) => {
    if (resp) {
      ...
      const ms = diffTime(hrStart),
        expressRoute = `${req.hostname}${req.baseUrl}${req.path}`;

      log('trace', `rendered express route: ${expressRoute} (${ms}ms)`, {
        action: 'render',
        route: expressRoute,
        renderTime: ms
      });
    ...
    if (error.name === 'NotFoundError') {
      log('trace', `in ${req.uri}: error.message`, {
        error: error.message
      });
      next();
    } else {
