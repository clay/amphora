...
function getExtension(req) {
  return req ? req.params.ext.toLowerCase() : renderers.default;
}
...
function renderComponent(req, res, options) {
  var extension = getExtension(req),
...
function renderPage(uri, res) {
    ...
    // TODO: Routes with no extensions (uris). Default to HTML or assign a default?
...
function renderUri(uri, res) {
  return db.get(uri).then(function (result) {
    ...
    if (route.isUri) {
      ...
      return route.default(result, res);
    }
  });
}
...
function renderExpressRoute(req, res, next) {
  ...
  renderUri(pageReference, res).then((resp) => {
    if (resp) {
