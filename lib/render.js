...
function renderComponent(req, res, hrStart) {
  ...
  return components.get(uri, locals)
    .then(cmptData => formDataForRenderer(cmptData, { ref: uri }, locals))
...
function formDataFromLayout(locals, uri) {
  return function (result) {
    ...
    return components.get(layoutReference, locals)
      ...
      .then(mappedData => module.exports.formDataForRenderer(mappedData, { layoutRef: layoutReference, ref: uri }, locals));
  };
}
 ...
 * @return {Promise}
 */
function formDataForRenderer(unresolvedData, options, locals) {
