...
describe(_.startCase(filename), function () {
  let sandbox, fakeLog, fakePassport;

  beforeEach(function () {
    sandbox = sinon.sandbox.create();
    fakeLog = sandbox.stub();
    fakePassport = sandbox.stub();
    fakePassport.addRoutes = sandbox.stub();

    lib.setLog(fakeLog);
    lib.setamphoraAuthentication(fakePassport);
  });
  ...
  describe('addSite', function () {
    ...
    it('inits auth if there are providers', function () {
      ...
      return fn(
        ...
        sinon.assert.called(fakePassport);
      });
    });
