 ...
 * request is attempting to archive the page. If true, the checkPublished
 * function is run.
 * @param {*} req
 * @param {*} res
 * @param {*} next
 */
function requestArchived(req, res, next) {
  req.body.archived
    ? checkPublished(req, res, next)
    : next();
}

/**
 * Checks the published property on the meta object of the page, by calling
 * checkProps with the argument 'published'.
 * @param {*} req
 * @param {*} res
 * @param {*} next
 */
function checkPublished(req, res, next) {
  metaController.checkProps(req.uri, 'published')
    .then(propsVal => {
      propsVal
        ? res.status(400).send('Please unpublish the page before attempting to modify the archived property')
        : next();
    })
...
function routes(router) {
  ...
  router.put('/:name/meta', requestArchived, route.putMeta);
