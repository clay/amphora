...
function publishPage(req, res) {
  responses.expectJSON(function () {
    ...
    return bluebird.map(refs, function (currRef) {
      /**
       ...
       */
      return components.get(currRef)
        .then(composer.resolveDataReferences)
        .then(references.replaceAllVersions(published))
        .then(function (currData) { return [references.replaceVersion(currRef, published), currData]; })
...
function createPage(req, res) {
  var body = req.body,
    ...
    currPageData = body && _.omit(body, 'layout'),
  ...
  responses.expectJSON(function () {
    return components.get(layoutReference).then(function (layoutData) {
      validatePageData(currPageData, layoutData);

      return cloneDefaultComponents(currPageData);
