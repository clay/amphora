...
function routes(router) {
  ...
  router.all('/pages', responses.methodNotAllowed({allow: ['get', 'post']}));
  router.all('/pages', responses.notAcceptable({accept: ['application/json']}));
  router.get('/pages', pageList.getPageList);  // List all pages
  router.post('/pages', pageList.searchPages); // Search all pages

  router.all('/sites', responses.methodNotAllowed({allow: ['get']}));
  router.all('/sites', responses.notAcceptable({accept: ['application/json']}));
  router.get('/sites', pageList.getSiteList);  // List all sites
}
