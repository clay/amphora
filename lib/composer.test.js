...
describe(_.startCase(filename), function () {
  ...
  it('passes Errors to Express', function (done) {
    var res = getMockRes(function () { done('should throw'); }),
      pathOnBase64 = '/uris/' + new Buffer(res.req.url).toString('base64');


    db.get.withArgs(pathOnBase64).returns(bluebird.reject(new Error('whatever')));
  ...
  it('gets page without query string', function (done) {
    ...
    res.req.url = '/some/url/here?some=query';
  ...
  it('renders page after following uri', function (done) {
    ...
    res.req.url = '/some/url/here?some=query';
    db.get.withArgs(pathOnBase64).returns(bluebird.resolve('/pages/a'));
    db.get.withArgs('/pages/a@published').returns(resolveString({layout: '/components/b'}));
    components.get.withArgs('/components/b').returns(bluebird.resolve({}));
    components.getTemplate.returns(template);
    plex.render.returns('<thing></thing>');


    lib(res.req, res, function (err) {
