...
describe(_.startCase(filename), function () {
  ...
  describe('renderPage', function () {
    ...
    it('prevents edits in view mode by redirecting', function () {
      const res = getMockRes(),
        req = getMockReq();

      req.query = { edit: 'true' };
      res.locals.url = 'foo.com/_pages/whatever@published.html?edit=true';

      fn('/_pages/whatever@published', req, res);
      sinon.assert.calledOnce(res.redirect);
      sinon.assert.calledWith(res.redirect, 'foo.com/_pages/whatever.html?edit=true');
      sinon.assert.callCount(db.get, 0);
      sinon.assert.callCount(res.send, 0);
      sinon.assert.callCount(components.get, 0);
