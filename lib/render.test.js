...
describe(_.startCase(filename), function () {
  ...
  describe('renderPage', function () {
    ...
    it('logs if there is an error', function () {
      const res = getMockRes(),
        req = getMockReq();

      db.get.returns(Promise.reject());

      return fn('/_pages/whatever', req, res).catch(function (e) {
        console.log(e);
        // sinon.assert.calledOnce(logFn);
        // sinon.assert.calledWith(logFn, 'error', sinon.match(/Error rendering/));
      });
    });
    ...
    it('follows the basic process', function () {
      ...
      return fn('/_pages/whatever', req, res).then(function () {
        ...
        sinon.assert.calledWith(logFn, 'info', sinon.match(/rendered page uri/));
      });
    });
  });
