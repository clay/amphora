...
function routes(router) {
  ...
  router.all('/_search/pages', responses.methodNotAllowed({allow: ['get', 'post']}));
  router.all('/_search/pages', responses.notAcceptable({accept: ['application/json']}));
  router.get('/_search/pages', pageList.getPageList);  // List all pages
  router.post('/_search/pages', pageList.searchPages); // Search all pages
  router.get('/_search/sites', pageList.getSiteList);  // Sites list
}
