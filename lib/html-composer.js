...
let uriRoutes, multiplex, renderers;
...
function renderTemplate() {
  throw new Error('You tried to call `renderTemplate`')
...
function getExtension(reqUrl) {
  return reqUrl.indexOf('.') > -1 ? reqUrl.split('.').pop() : 'html'; // Assume default to HTML
}
 ...
 */
function determineRenderer(data) {
  var reqUrl = _.get(data, 'locals.url'),
    extension = getExtension(reqUrl),
...
/**
 * TODO: renders
 * @param {[type]} renderObj [description]
 */
function addRenderers(renderObj) {
  renderers = renderObj;
}
...
module.exports.addRenderers = addRenderers;
