...
function applyOptions(options, res) {
  return function (result) {
    ...
    // todo: figure out a way to set this read-only without messing with the references,
    // since setting it read-only will actually break saving if the user saves fast enough
    // note: the reason why we'd want this to be read-only is to disallow templates from mucking
    // with the data as it's passed through them (that's the server.js's job)
    state.state = state;
