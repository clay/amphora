...
function getListFromDB(req, res) {
  expectJSON(function () {
    return db.get(req.uri).then(JSON.parse).then(d => JSON.parse(d.value));
  }, res);
}
...
function putRouteFromDB(req, res) {
  expectJSON(function () {
    return db.put(req.uri, JSON.stringify(req.body))
      .then(() => req.body);
  }, res);
}
...
module.exports.getListFromDB = getListFromDB;
