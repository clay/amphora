 ...
 * @returns {number}
 */
function checkPageSize(req, res) {
  try {
    let size;

    // fall back to default
    if (!req.query.size && process.env.CLAY_STORAGE_DEFAULT_PAGE_SIZE) {
      size = Math.abs(Number(process.env.CLAY_STORAGE_DEFAULT_PAGE_SIZE));
    } else if (req.query.size) {
      // invalid size query param
      if (isNaN(Math.abs(Number(req.query.size)))) {
        throw Error('Client: Invalid size');
      } else {
        // valid size query
    ...
    const max = Math.abs(Number(process.env.CLAY_STORAGE_MAX_PAGE_SIZE));
...
function checkPrevious(req, res) {
  try {
    ...
    if (previous) {
      if (previous.includes(prefix)) {
        return previous;
      } else {
        throw Error('Client: Invalid previous');
      }
    } else {
      return;
    }
