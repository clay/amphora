  ...
  return function (req, res) {
    let stream,
      listOptions = _.assign({
        prefix: clayUtils.uriPrefixToSlug(req.uri, res.locals.site),
        values: false
      }, options);
...
function listAndSwapPrefix(req, res) {
  var stream,
    ...
    options = {
      prefix: clayUtils.uriPrefixToSlug(req.uri, site),
      values: false,
      transforms() {
        return [
          ...
          map({ wantStrings: true }, function (str) {
            return clayUtils.uriSlugToPrefix(str, site);
          })
        ];
      }
    };
...
function convertDBListToReferenceObjects(site) {
  return function (obj) {
    ...
    item[referencedProperty] = clayUtils.uriSlugToPrefix(obj.key, site);
    return item;
  };
}
...
function getRouteFromDB(req, res) {
  expectJSON(function () {
    ...
    return db.get(clayUtils.uriPrefixToSlug(req.uri, site))
      .then(clayUtils.jsonSlugToPrefix(site))
      .then(JSON.parse);
  }, res);
}
...
function putRouteFromDB(req, res) {
  expectJSON(function () {
    var { locals: { site } } = res,
      dbUri = clayUtils.uriPrefixToSlug(req.uri, site),
      data = clayUtils.jsonPrefixToSlug(JSON.stringify(req.body), site);
...
function deleteRouteFromDB(req, res) {
  expectJSON(function () {
    const dbUri = clayUtils.uriPrefixToSlug(req.uri, res.locals.site);

    return db.get(dbUri)
      .then(clayUtils.jsonSlugToPrefix(res.locals.site))
      .then(JSON.parse)
