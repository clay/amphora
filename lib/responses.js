...
function removeExtension(path) {
  ...
  console.log('removeExtension', path, endSlash);

  if (endSlash > -1) {
    console.log('removeExtension', path.substr(0, path.indexOf('.', endSlash)));
  }
  return path;
}
 ...
 */
function getUri(req) {
  console.log('getUri', req.hostname + req.baseUrl + req.url);
  return normalizePath(req.hostname + req.baseUrl + req.url);
}
...
function getRouteFromDB(req, res) {
  expectJSON(function () {
    return db.get(getUri(req)).then(JSON.parse);
  }, res);
}
...
function putRouteFromDB(req, res) {
  expectJSON(function () {
    return db.put(getUri(req), JSON.stringify(req.body)).return(req.body);
  }, res);
}
