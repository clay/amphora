 ...
 * @returns {function}
 */
function listWithPublishedVersions() {
  const options = {
    transforms() {
      return [filter({ wantStrings: true }, function (str) {
        return str.indexOf('@published') !== -1;
      })];
    }
  };
...
/**
 * List all things in the db that start with this prefix
 * but check specific flags to determine how to filter the
 * response
 *
 * @param  {Object} req
 * @param  {Object} res
 */
function listByVersion(req, res) {
  if (req && req.query.published) {
    listWithPublishedVersions()(req, res);
  } else {
    listWithoutVersions()(req, res);
  }
}
...
module.exports.listByVersion = listByVersion;
