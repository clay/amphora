...
function addControllerRoutes(router) {
  ...
  _.each(files.getFiles(__dirname + '/controllers'), function (filename) {
    var pathContainer,
      ...
      controller = require('./routes/' + name);
 ...
 * @param [options]
 *   options.hostMap  (Optional) Switch hostname names with some different list
 *
...
function loadFromConfig(router) {
  var hostMap = config.get('hosts'); //includes local.yaml
