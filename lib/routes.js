...
const _ = require('lodash'),
  ...
  log = require('./services/log').setup({ file: __filename, amphoraProcess: 'routes' }),
  cors = {
    origins: '*',
...
function addAssetDirectory(site, maxAge = 0) {
  if (!files.fileExists(path.resolve(process.cwd(), site.assetDir))) {
    log('fatal', new Error(`Asset directory does not exist for ${site.slug}: ${site.assetDir}`));
    process.exit(1); // If no asset directory, nothing will work. Exit.
  }
