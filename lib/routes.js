...
var log = require('./services/logger').setup({ file: __filename });
...
function addSiteLocals(site) {
  return function (req, res, next) {
    ...
    res.locals.url = references.uriToUrl(req.hostname + req.originalUrl, site.protocol, site.port);
...
function addControllerRoutes(router) {
  ...
  _.each(reservedRoutes, function (routeName) {
    let pathRouter,
  ...
  if (site.dir) {
    ...
    if (controller) {
      if (Array.isArray(controller.routes)) {
        // pass in reservedRoutes to prevent site from overwriting an internal amphora api route
        attachRoutes(router, controller.routes, site, reservedRoutes);
      } else {
