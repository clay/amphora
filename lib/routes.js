...
function addHost(options) {
  // router, hostname, sites, providers, sessionStore
  ...
  router.use(vhost(options.hostname, hostRouter));
}
...
function loadFromConfig(router, providers, sessionStore) {
  ...
  _.each(siteHosts, function (hostname) {
    ...
    addHost({router: router, hostname: hostname, sites: sites, providers: providers, sessionStore:sessionStore});
  });
  return router;
}
