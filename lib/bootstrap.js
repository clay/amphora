...
function load(bootstrap, prefix) {
  ...
  promiseListsOps = saveObjects(prefix + '/lists/', bootstrap.lists, getPutOperation);
  promiseUrisOps = saveBase64Strings(prefix + '/uris/', bootstrap.uris, getPutOperation);
  _.forEach(bootstrap.uris, function (uri) {
    log.logLess('Bootstrapped uri: ' + uri);
  });
  promiseComponentsOps = saveWithInstances(prefix + '/components/', bootstrap.components, function (name, item) {
    return bluebird.join(
      components.getPutOperations(name, _.cloneDeep(item)),
      components.getPutOperations(name + '@published', _.cloneDeep(item))
    );
  });
  promisePagesOps = saveObjects(prefix + '/pages/', bootstrap.pages, function (name, item) {
    return [
  ...
  return bluebird.join(promiseComponentsOps, promisePagesOps, promiseUrisOps, promiseListsOps, function (componentsOps, pagesOps, urisOps, listsOps) {
    componentsOps = _.flattenDeep(componentsOps);
    pagesOps = _.flattenDeep(pagesOps);
    urisOps = _.flattenDeep(urisOps);
    listsOps = _.flattenDeep(listsOps);
    var ops = componentsOps.concat(pagesOps, urisOps, listsOps);
    return db.batch(ops, { fillCache: false, sync: false })
    .tap(search.batch(componentsOps));
  });
}
