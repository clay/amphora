...
function bootstrapComponents(site) {
  ...
  return bluebird.all(_.map(components, function (component) {
    ...
    return bootstrapPath(componentPath, site).catch(bootstrapError.bind(null, component));
  }));
}
...
function bootstrapError(component, e) {
  ...
  return bluebird.reject(_.clone(component));
}
...
function bootrapSitesAndComponents() {
  ...
  return highland(sitesArray)
    ...
    .toPromise(bluebird)
    .then(function (resp) {
      log('debug', `${sitesArray.length} sites bootstrapped with ${ERRORING_COMPONENTS.length} erroring components`);
