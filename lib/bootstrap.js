...
function load(data, site) {
  let promiseComponentsOps, promisePagesOps, promiseUrisOps, promiseListsOps, promiseUsersOps, promiseLayoutOps,
    prefix = site.prefix,
  ...
  promiseLayoutOps = saveWithInstances(`${prefix}/_layouts/`, compData._layouts, (name, item) => {
    return bluebird.join(
      layouts.getPutOperations(name, _.cloneDeep(item))
    ).then(_.flatten);
  });
  /* eslint max-params: ["error", 6] */
  return bluebird.join(promiseComponentsOps, promisePagesOps, promiseUrisOps, promiseListsOps, promiseUsersOps, promiseLayoutOps, (componentsOps, pagesOps, urisOps, listsOps, userOps, layoutOps) => {
    const flatComponentsOps = _.flatten(componentsOps),
      flatLayoutOps = _.flatten(layoutOps),
      ops = flatComponentsOps.concat(_.flattenDeep(pagesOps.concat(urisOps, listsOps, userOps)), flatLayoutOps);
...
function bootstrapLayouts(site) {
  return bluebird.all(_.map(files.getLayouts(), layout => {
    const layoutPath = files.getLayoutPath(layout);

    return module.exports.bootstrapPath(layoutPath, site).catch(bootstrapError.bind(null, layout));
  }));
}
