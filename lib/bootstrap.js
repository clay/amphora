...
function load(data, site) {
  ...
  promiseListsOps = saveObjects(prefix + '/_lists/', compData.lists, getPutOperation);
  promiseUrisOps = saveBase64Strings(prefix + '/_uris/', compData.uris, getPutOperation);
  _.each(compData.uris, function (page, uri) {
    log('info', `bootstrapped: ${chalk.blue(uri)} ${chalk.dim(' => ')} ${chalk.dim(page)}`);
  });
  promiseComponentsOps = saveWithInstances(prefix + '/_components/', compData.components, function (name, item) {
    return bluebird.join(
      components.getPutOperations(name, _.cloneDeep(item))
    ).then(_.flatten);
  });
  promisePagesOps = saveObjects(prefix + '/_pages/', compData.pages, function (name, item) {
    return [
