...
function load(bootstrap, prefix) {
  ...
  return bluebird.join(componentsOps, pagesOps, urisOps, listsOps, function (ops0, ops1, ops2, ops3) {
    ops0 = _.flattenDeep(ops0);
    ops1 = _.flattenDeep(ops1);
    ops2 = _.flattenDeep(ops2);
    ops3 = _.flattenDeep(ops3);
    var ops = ops0.concat(ops1).concat(ops2).concat(ops3);
    return db.batch(ops, { fillCache: false, sync: false })
      .then(function(){
        return search.batchDBFormat(components.mapSearchBatchOps(ops0))
          .return(ops);
       });
  });
}
