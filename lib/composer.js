...
function composer(req, res, next) {
  var site = siteService.sites()[res.locals.site],
    prefix = site.host + site.path + '/uris/',
    pageReference = prefix + new Buffer((req.hostname + req.originalUrl).split('?').shift()).toString('base64');

  console.log('uri:', req.uri, prefix, res.locals.site, req.hostname + req.originalUrl);
