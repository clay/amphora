...
/**
 * If ref can be routed via uriRoutes, then do so locally.
 * If not, then assume a link elsewhere, so fetch.
 *
 * @param {string} ref
 * @returns {Promise}
 */
function getFromRoutingSource(ref) {
  var handler = _.first(uriRoutes, function (item) { return ref.match(item.when); });

  if (handler && _.isFunction(handler.json)) {
    return handler.json(ref);
  } else {
    return fetch(ref).then(function (result) { return result.json(); });
  }
}
...
function resolveDataReferences(data, version) {
  ...
  return bluebird.all(placeholders).each(function (placeholder) {
    var promise,
      ...
      handler = _.first(uriRoutes, function (item) { return ref.match(item.when); });

    if (handler && _.isFunction(handler.json)) {
      //replace with parent version
      ref = replaceVersion(ref, version);
      promise = handler.json(ref);
    } else {
...
function renderComponent(ref, res, options) {
  ...
  is(res.locals, 'res.locals');
  is(res.locals.site, 'res.locals.site');
  is(ref, 'component reference');
  var componentName = is(components.getName(ref), 'component name');
...
function renderPage(ref, res) {
  ...
  return db.get(ref)
    ...
    .then(function (result) {
      ...
      return components.get(replaceVersion(layoutReference, version))
 ...
 */
uriRoutes = [
  { when: /^\/pages\//, handler: renderPage, json: db.getObject },
  { when: /^\/components\//, handler: renderComponent, json: components.get },
  { when: /^\/uris\//, handler: renderUri, json: db.get }
];
