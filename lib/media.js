...
function getContentsOfFiles(fileArray, targetDir, filePrefix) {
  ...
  fileArray.forEach(file => {
    allPromises.push(files.readFilePromise(path.join(rootDir, targetDir, getFileName(file, filePrefix))));
  });
...
function combineStyles(stylesArr) {
  ...
  return getContentsOfFiles(stylesArr, 'public/css', '/css/')
    .then(data => '<style>' + data.join('') + '</style>');
}
...
function combineScripts(scriptsArr) {
  ...
  return getContentsOfFiles(scriptsArr, 'public/js', '/js/')
    .then(data => '<script type="text/javascript">' + data.join('') + '</script>');
}
