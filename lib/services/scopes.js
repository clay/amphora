 ...
 * @param {Object} locals
 * @return {Stream} of an object that map cmpt URIs to Elastic results
 */
...
function scopesToTasks(scopes, locals) {
  return h.pairs(scopes)
    ...
    .tap(task => task.pinned = !!task.query._pinned)
...
function executeGeneration(generation) {
  return h(generation)
    .map(({query, exclude, pinned}) => pinned ?
      query : // ignore exclusions if pinned
      addExclusions(query, exclude)
    )
    .reduce({body: []}, (prev, curr) => {
      prev.body.push(
        _.omit(curr, ['body', '_pinned']),
        curr.body
      );
      return prev;
    })
