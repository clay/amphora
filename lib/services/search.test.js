...
describe(_.startCase(filename), function () {
  ...
  describe('update', function () {
    ...
    it('calls the update method provided by the ES client', function () {
      client.update.returns(Promise.resolve('ES Client `put` called'));
      return fn('index', 'type', 'some/ref', { some: 'data' })
        .then(function () {
  ...
  describe('getDocument', function () {
    ...
    it('calls the get method provided by the ES client', function () {
      client.get.returns(Promise.resolve('ES Client `put` called'));
      return fn('index', 'type', 'some/ref')
        .then(function () {
  ...
  describe('existsDocument', function () {
    ...
    it('calls the exists method provided by the ES client', function () {
      client.exists.returns(Promise.resolve('ES Client `put` called'));
      return fn('index', 'type', 'some/ref')
        .then(function () {
  ...
  describe('retrieveMappingData', function () {
    ...
    it('returns a mapping object if the file exists', function () {
      ...
      files.getFiles.returns(['pages.yml']);
      files.getYaml.returns({ pages: { mapping: true } });
      result = fn('path');
      expect(result).to.deep.equal({ pages: { pages: { mapping: true } } });
    });
  ...
  describe('createInternalIndices', function () {
    var fn = lib[this.title];

    it('requires a module if it exists', function () {
      sandbox.stub(path, 'resolve').returns('/path/to/elastic/mappings');
      sandbox.stub(lib, 'retrieveMappingData').returns(Promise.resolve({
        pages: {
          general: {
            dynamic: 'strict',
            properties: {
      ...
      sandbox.stub(pageList, 'sitesIndex').returns(Promise.resolve());
      sandbox.stub(db, 'on');

      return fn()
        .then(function(resp) {
          expect(db.on.calledOnce).to.be.true;
        });
    });
