...
function connect(busModule) {
  // set on every connection, not just the injected one. reconnecting without a module used to leave
  // this true, so `publish` kept taking the injected path against a raw ioredis client: it adopted
  // that client's rejections and handed it an unserialized object.
  BUS_MODULE = Boolean(busModule);
 ...
 *
 * Only an injected bus module's Promise is adopted. When a `busModule` was
 * passed to `connect`, whatever its `publish` returns is returned here, so
 * callers that chain on it (`pages.applyBatch`, `db-operations.cascadingPut`)
 * wait on the bus module and its rejections propagate to them. That only buys
 * propagation if the module's `publish` actually returns its promise.
 *
 ...
 * @throws {Error} synchronously, on a missing or wrongly typed topic or msg
 */
