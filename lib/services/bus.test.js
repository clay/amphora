 ...
 *
 * `connect` sets a module-level flag when it is handed a bus module, and never
 * unsets it, so a single `connect(busModule)` call would otherwise decide which
 * branch of `publish` every later test takes -- in this file and in every other
 * test file that shares the cached module. Tests that care about a specific
 * branch use their own copy, which keeps them independent of declaration order
 * and file order. The shared cached instance (the one `pages`, `db-operations`
 * and the other test files already hold a reference to) is left untouched.
 *
 * @returns {Object}
 */
function freshBus() {
