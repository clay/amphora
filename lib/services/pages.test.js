...
describe(_.startCase(filename), function () {
  ...
  describe('create', function () {
    const fn = lib[this.title],
      locals =  { site: { slug: 'domain', prefix: 'domain.com/path', host: 'domain.com', path: '/path' }};
    ...
    it('creates without content', function () {
      ...
      return fn('domain.com/path/_pages', {layout: 'domain.com/path/_components/thing'}, locals).then(function (result) {
    ...
    it('creates with content', function () {
      ...
      components.get.onFirstCall().returns(bluebird.resolve(layoutReferenceData));
      components.get.withArgs(contentUri).returns(bluebird.resolve(contentData));
      db.batch.returns(bluebird.resolve());
      siteService.getSiteFromPrefix.returns({notify: _.noop});

      return fn(uri, data, locals).then(function (result) {
    ...
    it('creates with content with inner references', function () {
      const uri = 'domain.com/path/_pages',
        contentUri = 'domain.com/path/_components/thing1',
        layoutUri = 'domain.com/path/_components/thing2',
        innerContentUri = 'domain/_components/thing3',
      ...
      return fn(uri, data, locals).then(function (result) {
        ...
        expect(batchOps[0].key).to.match(new RegExp('domain/_components/thing4/instances/'));
        expect(batchOps[0].type).to.equal('put');
        expect(JSON.parse(batchOps[0].value)).to.deep.equal(innerContentInstanceData);

        expect(batchOps[1].key).to.match(new RegExp('domain/_components/thing1/instances/'));
        expect(batchOps[1].type).to.equal('put');
        expect(JSON.parse(batchOps[1].value).thing).to.deep.equal({_ref: innerContentUri});
        expect(JSON.parse(batchOps[1].value).instanceThing._ref).to.match(new RegExp('domain/_components/thing4/instances/'));

        expect(batchOps[2].key).to.match(new RegExp('domain/_pages/'));
        expect(batchOps[2].type).to.equal('put');
        expect(JSON.parse(batchOps[2].value).layout).to.equal('domain/_components/thing2');
        expect(JSON.parse(batchOps[2].value).content).to.match(new RegExp('domain/_components/thing1/instances/'));
      });
    });
  });
  ...
  describe('publish', function () {
    ...
    it('creates relevant uri', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {layout: 'domain.com/path/_components/thing'},
        {
          site: {
            slug: 'domain',
        ...
        expect(secondLastOp).to.deep.equal({
          type: 'put',
          key: 'domain/_uris/c29tZS1kb21haW4uY29tLw==',
          value: 'domain/_pages/thing'
        });
      });
    });
    ...
    it('warns if notification fails', function () {
      ...
      return fn(uri, {layout: 'domain.com/path/_components/thing'},
        {
          site: {
            resolvePublishUrl: [ () => Promise.resolve('http://some-domain.com') ]
          }
    ...
    it('publishes without provided data', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {}, {
        ...
        expect(result).to.deep.equal({layout: 'domain.com/path/_components/thing@published', url: 'http://some-domain.com', urlHistory: ['http://some-domain.com']});
      });
    });

    it('throws when a site does not provide a resolvePublishUrl function to modify the publishing chain', function (done) {
      components.get.returns(bluebird.resolve({}));
      db.batch.returns(bluebird.resolve());
      siteService.getSiteFromPrefix.returns({notify: _.noop});
      notifications.notify.returns(bluebird.resolve());
