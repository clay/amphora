  ...
  describe('publish', function () {
    ...
    it('creates relevant uri', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {layout: 'domain.com/path/_components/thing'},
        {
          site: {
            resolvePublishUrl: () => [ function (uri, data) { data.url = 'http://some-domain.com'; return data; } ]
          }
        }).then(function () {
    ...
    it('warns if publish is slow', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {layout: 'domain.com/path/_components/thing'},
        ...
        sinon.assert.calledWith(fakeLog, 'warn', sinon.match('slow publish domain.com/path/pages/thing@published 700ms'));
      });
    });
    ...
    it('logs if publish is not slow', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {layout: 'domain.com/path/_components/thing'},
        ...
        sinon.assert.calledWith(fakeLog, 'info', sinon.match('published domain.com/path/pages/thing 20ms'));
      });
    });
    ...
    it('publishes without provided data', function () {
      ...
      return fn('domain.com/path/_pages/thing@published', {}, {
        site: {
          resolvePublishUrl: () => [ function (uri, data) { return Promise.resolve('http://some-domain.com'); } ]
        }
      }).then(function (result) {
        expect(result).to.deep.equal({layout: 'domain.com/path/_components/thing@published', url: 'http://some-domain.com'});
      });
    });
    ...
    it('throws when a sites publishing chain does not provide a url', function (done) {
      ...
      fn('domain.com/path/_pages/thing@published', {layout: 'domain.com/path/_components/thing'},
        { site: {resolvePublishUrl: () => [ function (uri, data) { return data; } ]} }).then(done)
        .catch(function (result) {
