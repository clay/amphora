...
describe(_.startCase(filename), function () {
  ...
  afterEach(function () {
    sandbox.restore();
    // the bus module singleton's client isn't sandbox-managed, so reset it to
    // its default in case a test set a fake one
  ...
  describe('publish', function () {
    ...
    // Uses the real `bus.publish` against a rejecting bus client instead of a
    // stubbed rejection, so this also fails if `bus.publish` ever goes back to
    // being fire-and-forget and swallowing its client's errors.
    //
    // It goes through `bus.connect(fakeBus)` because only an injected bus
    // module's Promise is returned by `bus.publish`; the default ioredis client
    // stays fire-and-forget on purpose (see lib/services/bus.js), so setting
    // `bus.client` alone would exercise the branch that never rejects. Note that
    // `connect` flips a module-level flag for the rest of the process, which is
    // why this connects explicitly rather than leaning on ambient state.
    it('rejects with the error from the `save` publish to the bus', function () {
      const pageData = {layout: 'domain.com/path/_layouts/thing'},
        busError = new Error('Client: could not index page'),
        busClientPublish = sandbox.stub().rejects(busError),
        fakeBus = { connect: () => ({ publish: busClientPublish })};
      ...
      meta.publishPage.returns(Promise.resolve());
      // un-stub, so the real connect and publish run (see above)
      bus.connect.restore();
      bus.publish.restore();
      bus.connect(fakeBus);

      return fn('domain.com/path/_pages/thing@published', pageData, locals).then(
        () => { throw new Error('expected publish to reject'); },
        result => {
          expect(result).to.equal(busError);
          expect(result.message).to.equal('Client: could not index page');
          // the batch write already happened when the rejection surfaced, and
          // nothing after the failed `save` publish ran
          sinon.assert.calledOnce(db.batch);
          sinon.assert.calledWith(busClientPublish, sinon.match(/save$/));
          sinon.assert.notCalled(meta.publishPage);
