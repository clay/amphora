...
const elastic = require('elasticsearch'),
  ...
  _ = require('lodash'),
  elasticHelpers = require('../utils/elastic-helpers'),
  db = require('./db'),
...
/**
 * Setup the client and check the health of the client
 *
 * @return {Promise}
 */
function setup() {
...
function validateIndices(mappings) {
  return bluebird.all(_.map(Object.keys(mappings), function (index) {
    return createIndexIfNone(createIndexName(index)).then(function () {
      return createAliasIfNone(index);
    });
  }))
  .then(function () {
    return bluebird.all(_.reduce(mappings, function (list, types, index) {
      return list.concat(_.map(types, function (mapping, type) {
        return createMappingIfNone(createIndexName(index), type, mapping);
      }));
    }, []));
  });
}
...
/**
 * Query Elastic with a payload provided
 * by the user via a POST to the `/search`
 * endpoint. User is responsible for sending
 * the proper payload
 *
 * @param {object} queryObj [description]
 * @returns {Promise}
 */
function RESTQuery(queryObj) {
  return client.search(queryObj);
}
 ...
 */
function retrieveMappingData(dirPath) {
...
function createInternalIndices() {
  var dirPath = path.resolve(__dirname, '../mappings'),
    internalMappings = retrieveMappingData(dirPath);

  // TODO: check for folder and index file inside of Sites
  return validateIndices(internalMappings)
    .then(require(process.cwd() + '/search'))
    .then(function() {
      // Listen to events internally
      db.on('batch', pageList.updatePageList(internalMappings));
      // Listen to events in the Clay instance
      // Resolve the promise
      return bluebird.resolve();
    });
}
...
module.exports.filters = require('../utils/db-filters');
// Elastic Helpers
...
module.exports.registerCustomTypes = elasticHelpers.registerCustomTypes;
