...
function existsMapping(index, type) {
  return client.indices.getMapping({
    ...
    .then(function (mapping) {
      let getMapping = _.get(mapping, index + '.mappings.' + type);
  ...
  return existsAlias(index)
    .then(function (exists) {
      if (!exists) {
        // Indices should have a different name than the alias
        // Append '_v1' to newly created indices
        return initAlias(name, createIndexName(index))
          .then(function (result) {
 ...
 * @returns {Promise}
 */
function createMappingIfNone(index, type) {
  return existsMapping(index, type)
    .then(function (index, type) {
      if (!_.size(index)) {
        log('info', 'Mapping is missing!');
        return initMapping(index, type);
      } else {
...
/**
 * Filter which indices and ops to batch update to ES
 * @param {Array} ops, e.g. DB data
 * @param {Array} indices
 * @returns {Promise}
 */
function opsBatch(ops, indices, filters) {
  return bluebird.all(_.map(_.pick(mappings, indices), function (types, index) {
    return bluebird.all(_.map(types, function (mapping, typeName) {
      let ops = _.cloneDeep(ops);

      ops = _.map(ops, convertOpValueToObject);

      if (ops.length > 0) {
        // this potentially looks up refs
        return normalizeOpValuesWithMapping(mapping, ops).then(function (normalizedOps) {
          // now remove any refs remaining; they're not allowed in search results yet.
          normalizedOps = _.map(normalizedOps, removeAllReferences);

          return batch(convertRedisBatchtoElasticBatch(index, typeName, normalizedOps));
        });
      }
    }));
  }));
