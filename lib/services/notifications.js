 ...
 * @returns {Promise}
 */
function callWebhook(event, url, data, uri) {
  const headers = {
      'X-Event': event,
      'page-uri': uri
    },
 ...
 * @returns {Promise}
 */
function notify(site, eventName, data, uri) {
  const events = _.get(site, 'notify.webhooks');

  if (events && _.isArray(events[eventName])) {
    return bluebird.all(_.map(events[eventName], function (url) { return callWebhook(eventName, url, data, uri); }));
  }
