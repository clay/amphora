...
describe(_.startCase(filename), function () {
  ...
  describe('logged get wrapping', function () {
    it('wraps `get` in a logging function that warns if called without a key', () => {
      ...
      lib.get('');
      sinon.assert.calledWith(mockLog, 'warn');
    });

    it('does not log if `get` is called with a key', () => {
      const mockLog = sinon.stub();
      ...
      lib.get('some-key');
      sinon.assert.notCalled(mockLog);
    });
  });
});
