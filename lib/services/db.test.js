...
describe(_.startCase(filename), function () {
  ...
  describe('put', function () {
    ...
    it('executes save and publish plugin hooks for @published', function () {
      return fn('key@published','val').then(() => {
        expect(plugins.executeHook.firstCall.args).to.deep.equal([
          'save', 'key@published', 'val'
        ]);
        expect(plugins.executeHook.secondCall.args).to.deep.equal([
          'publish', 'key@published', 'val'
        ]);
      });
    });
  });
  ...
  describe('batch', function () {
    ...
    it('executes save plugin hook', function () {
      ...
      return fn(ops).then(() => {
        expect(plugins.executeHook.firstCall.args).to.deep.equal(['save', ops]);
      });
    });
    it('executes save and publish plugin hooks for @published', function () {
      ...
      return fn(ops).then(() => {
        expect(plugins.executeHook.firstCall.args).to.deep.equal(['save', ops]);
        expect(plugins.executeHook.secondCall.args).to.deep.equal(['publish', ops]);
      });
    });
  });
