...
function publishExternally(url) {
  return bluebird.try(function () {
    const published = clayUtils.replaceVersion(url, 'published');
 ...
 */
function createScheduleObjectKey(data, slug) {
  const at = data[scheduledAtProperty],
  ...
  return `${slug}/_schedule/${buf.encode(publish.replace(/https?:\/\//, ''))}`;
}
...
function del(uri, site) {
  const dbUri = clayUtils.uriPrefixToSlug(uri, site);

  return db.get(dbUri).then(JSON.parse).then(function (data) {
    const targetUri = clayUtils.uriPrefixToSlug(references.urlToUri(data[publishProperty]), site),
      targetReference = clayUtils.replaceVersion(targetUri, scheduledVersion),
      ops = [
        { type: 'del', key: dbUri },
 ...
 * @returns {Promise}
 */
function post(uri, data, { site }) {
  const reference = createScheduleObjectKey(data, site.slug),
    targetUri = references.urlToUri(data[publishProperty]),
    targetReference = clayUtils.uriPrefixToSlug(clayUtils.replaceVersion(targetUri, scheduledVersion), site),
