...
function del(uri, site) {
  const dbUri = references.uriSwapInSlug(uri, site);

  return db.get(dbUri).then(JSON.parse).then(function (data) {
    const targetUri = references.uriSwapInSlug(references.urlToUri(data[publishProperty]), site),
...
function post(uri, data, { site }) {
  const reference = createScheduleObjectUID(data, site.slug),
    ...
    targetReference = references.uriSwapInSlug(clayUtils.replaceVersion(targetUri, scheduledVersion), site),
