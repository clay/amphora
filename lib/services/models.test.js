...
describe(_.startCase(filename), function () {
  ...
  let sandbox, fakeLog, savedTimeoutConstant;
  ...
  describe('get', function () {
    ...
    it('runs the renderer model if one is defined', function () {
      ...
      db.get.returns(Promise.resolve('{}'));
      upgrade.init.returns(Promise.resolve({}));
      return fn(_.noop, { render: rendererModel }, false, 'someuri', {})
        .then(() => {
    ...
    it('errors if the upgrade/renderer model do not return an object', function () {
      const render = sandbox.stub().returns(Promise.resolve({}));

      db.get.returns(Promise.resolve('{}'));
      upgrade.init.returns(() => Promise.resolve('somestring'));
      return fn({ render }, undefined, false, 'someuri', {})
        .catch(e => {
          expect(e.message).to.match(/Client: Invalid data type/);
        });
    });
