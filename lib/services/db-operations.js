...
function getPutOperations(putFn, uri, data, locals) {
  ...
  const layouts = splitCascadingData(uri, replacePropagatingVersions(uri, data));
  ...
  return bluebird.map(layouts, ({ key, value }) => putFn(key, value, locals))
