 ...
 */
function resolveComponentReferences(uri, data, locals, filter = referenceProperty) {
  const referenceObjects = references.listDeepValuesByKey(data, filter),
    schemaPromise = Object.keys(referenceObjects).length && (isComponent(uri) || isLayout(uri)) ? getSchema(uri) : bluebird.resolve();
    ...
    return bluebird.each(orderedRefs, ([, referenceObject]) => {
      ...
      return components.get(ref, locals)
        .then(obj => {
          ...
          return resolveComponentReferences(ref, obj, locals, filter).finally(() => {
 ...
 */
function composePage(uri, pageData, locals) {
  ...
  return components.get(layoutReference)
    ...
    .then(fullData => resolveComponentReferences(uri, fullData, locals));
}
