...
const _ = require('lodash'),
  ...
  { getPrefix, getPageInstance } = require('clayutils');
...
function mergeNewAndOldMeta(old, updated) {
  ...
  return Object.assign(old, updated, { history: old.history.concat(updated.history), firstPublishTime: old.firstPublishTime || updated.publishTime });
}
...
function putMeta(uri, newData) {
  var id = `${getPrefix(uri)}/_pages/${getPageInstance(uri).split('/')[0]}`;

  return getMeta(id).then(existing => db.putMeta(id, references.pickPageMeta(mergeNewAndOldMeta(existing, newData))));
}
 ...
 * @returns {Promise}
 */
function getMeta(uri, data) {
  var id = `${getPrefix(uri)}/_pages/${getPageInstance(uri).split('/')[0]}`;

  return db.getMeta(id, data)
    .catch(() => ({}));
}
