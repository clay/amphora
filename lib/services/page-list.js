...
function updatePageList(mappings) {
  return function (batchOps) {
    return search.applyOpFilters(batchOps, mappings, index, filterForPageOps)
      .then(function (resp) {
        var resp = resp[0][0],
...
function getPage(pageUri) {
  return search.getDocument('pages', 'general', pageUri);
}
...
function pageExists(ops) {
  return bluebird.all(_.map(ops, function (op) {
    return search.existsDocument('pages', 'general', pageUriFromKey(op.key));
  }));
}
...
function updatePageData(pageUri, data) {
  ...
  return search.update('pages', 'general', pageUri, data)
...
function queryPages(reqBody) {
  return function () {
    return search.query('pages', validatePayload(reqBody), 'general')
      .then(resp => resp.hits);
  }
}
