...
const bluebird = require('bluebird'),
  _ = require('lodash'),
  sites = require('./sites'),
  filters = require('../utils/db-filters'),
...
function updatePageList(mappings) {
  return function (batchOps) {
    return bluebird.all(_.map(_.pick(mappings, ['editable-pages']), function (types, indexName) {
      return bluebird.all(_.map(types, function (mapping, typeName) {
        ...
        if (ops.length > 0) {
          var test = constructPageData(mapping, ops);
          return search.batch(elasticHelpers.convertRedisBatchtoElasticBatch(indexName, typeName, test));
        }
      }));
    }));
  }
