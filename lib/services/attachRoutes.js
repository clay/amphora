...
function attachRedirect(router, { path, redirect }, site) {
  const dynamicRedirectRegExp = new RegExp(':([^\/]*)', 'ig');

  redirect = normalizeRedirectPath(redirect, site);

  if (path.match(dynamicRedirectRegExp)) {
    return router.get(path, (req, res) => {
      res.redirect(301, redirect.replace(dynamicRedirectRegExp, (match, p1) => req.params[p1]));
    });
  }
  return router.get(path, (req, res) => {
