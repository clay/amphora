...
/**
 * Handles the route attaching for intermediate middleware.
 * @param {Router} router
 * @param {Object} routeConfig
 * @param {String} routeConfig.path
 * @param {Function} routeConfig.middle
 */
function handleIntermediateMiddleware(router, { path, middle }) {
  if (middle) { // Look up for intermediate middleware to be attached
    router.get(path, middle);
  }
}
...
function parseHandler(router, routeObj, site) {
  ...
  handleIntermediateMiddleware(router, routeObj);
