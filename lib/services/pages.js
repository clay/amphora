...
/**
 * Prepare a published page for re-processing through the event bus.
 * @param {string} uri
 * @param {object} [data]
 * @param {object} [locals]
 * @param {object?} opts
 * @param {string} opts.pageUri: An optional pageUri to include in the operation.
 * @returns {Promise<object[]>}
 */
function reindex(uri, data, locals, { pageUri } = {}) {
  const componentList = references.getPageReferences(data);

  return bluebird.map(componentList, getRecursivePublishedPutOperations(locals))
    .then(_.flatten)
    .then(ops => addOp(uri, data, ops))
    .then(ops => ops.map((op) => ({ ...op, pageUri })));
}
