...
function getPageClonePutOperations(pageData, locals) {
  return bluebird.all(_.reduce(pageData, function (promises, pageValue, pageKey) {
    if (typeof pageValue === 'string' && getComponentName(pageValue)) {
      ...
      promises.push(components.get(pageValue, locals).then(refData => {
  ...
  return function (ops) {
    ...
    return db.batch(ops).then(() => {
      ...
      if (_.endsWith(lastOp.key, '@published')) {
        notifications.notify(site, 'published', lastValue).catch(error => {
...
function getLatestData(uri) {
  return db.get(replaceVersion(uri));
}
...
function putLatest(uri, data, locals) {
  ...
  if (!data.layout || !isLayout(data.layout)) {
    throw Error('Page must contain a `layout` property whose value is a `_layout` instance');
  }
