 ...
 * @returns {string}
 */
function renameReferenceUniquely(ref) {
  var prefix = ref.substr(0, ref.indexOf('/components/'));

  return prefix + '/components/' + components.getName(ref) + '/instances/' + uid();
}
...
function create(ref, data) {
  var layoutReference = data && data.layout,
    ...
    prefix = ref.substr(0, ref.indexOf('/pages')),
