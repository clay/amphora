...
function addOpToMakePublic(ops, sitePrefix, publicUrl, pageUri) {
  ...
  ops.push({
    type: 'put',
    key: sitePrefix + '/uris/' + new Buffer(references.urlToUri(publicUrl)).toString('base64'),
  ...
  return getPublishData(uri, data)
    .then(function (pageData) {
      if ( publishingChain.length > 0 ) {
        ...
        return bluebird.any( publishingChain.map( publishFunction => publishFunction(uri, pageData, locals) ) )
            .catch( () => pageData );
      }
