...
function renameReferenceUniquely(uri) {
  ...
  return prefix + '/_components/' + utils.getName(uri) + '/instances/' + uid.get();
}
...
function publish(uri, data, locals) {
  const startTime = process.hrtime(),
    ...
    dbUri = references.uriSwapInSlug(uri, site);
    ...
    .then(function (pageData) {
      ...
      return bluebird.map(componentList, getRecursivePublishedPutOperations(locals))
        ...
        .then(function (ops) {
          ...
          return addOp(references.replaceVersion(dbUri, published), pageData, ops);
        });
...
function create(uri, data, locals) {
  const site = locals && locals.site,
    layoutReference = data && references.uriSwapInSlug(data.layout, site),
