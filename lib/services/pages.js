...
function makeNewPage(site) {
  return function (ops) {
    const lastOp = _.last(ops),
      lastValue = lastOp && JSON.parse(lastOp.value);

    return db.batchNew(ops).return(lastValue);
  };
}
...
function create(uri, data, locals) {
  ...
  }).then(makeNewPage(site)).then(newPage => {
...
function putLatest(uri, data, locals) {
  ...
  return isLayout(data.layout).then((layoutIsLayout) => {
    ...
    return getLatestData(uri)
      ...
      .catch(() =>
      db.put(uri, JSON.stringify(data)).then(() => plugins.executeHook('createPage', { uri, data, user: locals && locals.user })).then(() => data));
  });
}
