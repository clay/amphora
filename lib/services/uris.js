...
function del(uri, user) {
  return db.get(uri).then(oldData => {
    return db.del(uri).then(() => {
      ...
      bus.publish('unpublishPage', JSON.stringify({ uri: oldData, url: pageUrl, user }));

      notifications.notify(site, 'unpublished', { url: pageUrl, uri: oldData });

      return pageMeta.unpublishPage(uri, user)
        .then(() => oldData);
    });
  });
}
