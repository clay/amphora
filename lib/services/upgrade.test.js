...
describe(_.startCase(filename), function () {
  ...
  describe('upgradeData', function () {
    ...
    it('does not try to run transforms if the versions are the same', function () {
      ...
      return fn(1, 1, 'site/components/foo/instances/bar', _.cloneDeep(sampleData))
        .then(function () {
    ...
    it('runs transforms if they should be run', function () {
      ...
      return fn(2, 1, 'site/components/foo/instances/bar', _.cloneDeep(sampleData))
        .then(function (resp) {
    ...
    it('logs an error and does not upgrade data if an upgrade fails', function () {
      const badUpgrade = sandbox.stub().throws(),
        cmptUri = 'site/components/foo/instances/bar';
      ...
      return fn(2, 1, cmptUri, _.cloneDeep(sampleData))
        .then(function (resp) {
          sinon.assert.calledWith(fakeLog, 'error', `Error upgrading ${cmptUri} to 2.0`);
          expect(resp._version).to.equal(1);
        });
    });
  });
});
