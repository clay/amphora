...
function addToHookOps(hookOps, op) {
  if (hookOps[op.type]) {
    hookOps[op.type].push(_.pick(op, 'key', 'value'));
  }
  return hookOps;
}
...
function triggerPlugins(method, args) {
  const hookOps = dbOpsToHookOps(method, args),
    ...
    isPublishedPageBatch = method === 'batch' && hookOps.put.length > 1 &&  _.includes(lastPutKey, '/pages/') && _.endsWith(lastPutKey, publishedVersionSuffix);

  if (hookOps.put.length) {
    plugins.executeHook('save', hookOps.put);
    if (isPublishedPageBatch) {
      plugins.executeHook('publish', hookOps.put);
    }
  }
...
/**
 * notifies outside listeners through eventify and amphora plugins
 * @param {string} method
 */
