...
function list(options) {
  ...
  readStream = mongo.createReadStream(options);
 ...
 */
function batch(ops) {
  var opsObj = makeMongoBatch(ops),
    promises = [];

  for (let key in opsObj) {
    if (opsObj[key].length) {
      for (let i = 0; i < opsObj[key].length; i++) {
        let val = opsObj[key][i];

        promises.push(mongo.updateMany(`${key}Collection`, val._id, val));
      }
...
function batchNew(ops) {
  var opsObj = {},
    promises = [];

  validation.assertValidBatchOps(ops);
  opsObj = makeMongoBatch(ops);

  for (let key in opsObj) {
    if (opsObj[key].length) {
      promises.push(mongo.insertMany(`${key}Collection`, opsObj[key]));
    }
  }
...
module.exports.batchNew = batchNew;
