...
/**
 * Intermediary logger to catch empty string keys and log an informative stack trace
 *
 * @param {Function} dbGet
 * @returns {Function}
 */
function loggedGet(dbGet) {
  return (key) => {
    if (!key) {
      const e = new Error('Attempted to get empty string');

      log('warn', e.message, { stack: e.stack, action: 'amphora-empty-get' });
    }
 ...
 * @param  {[boolean]} testSkipLoggedGet
 */
function registerStorage(storage, testSkipLoggedGet) {
  for (let action in storage) {
    if (testSkipLoggedGet) {
      module.exports[action] = storage[action];
    } else {
      module.exports[action] = action === 'get' ? loggedGet(storage[action]) : storage[action];
    }
  }
}
...
module.exports.loggedGet = loggedGet;
