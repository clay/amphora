...
function runTransforms(transforms, upgradeFile, ref, data, locals) {
  ...
  return bluebird.reduce(transforms, function (acc, val) {
    ...
    return bluebird.try(upgradeFile[key].bind(null, ref, acc, locals))
      .catch(e => {
        log('error', `Error upgrading ${ref} to ${key}`);
    ...
    .catch(function (e) {
      log('error', `Error saving upgraded component data for ${ref}: ${e.message}`);
...
function upgradeData(schemaVersion, dataVersion, ref, data, locals) {
  const componentName = utils.getName(ref),
    componentDir = files.getComponentPath(componentName), // Get the component directory
    upgradeFile = files.tryRequire(path.resolve(componentDir, 'upgrade')); // Grab the the upgrade.js file

  var transforms = [];


  log('info', `Running upgrade for ${componentName}: ${ref}`);
  // If no upgrade file exists, exit early
  if (!upgradeFile) {
    log('debug', `No upgrade file found for component: ${componentName}`);
  ...
  if (!transforms.length) {
    log('debug', `Upgrade tried to run, but no upgrade function was found for ${componentName}`, {
  ...
  return runTransforms(transforms, upgradeFile, ref, data, locals)
    ...
    .catch(e => {
      return bluebird.resolve(data); // if failed, resolve with the old data. Hopefully it won't break further down the line
    });
}
