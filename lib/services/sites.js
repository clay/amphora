...
function getSiteFromPrefix(prefix) {
  var split = prefix.split('/'), // Split the url/uri by `/`
    ...
    optPath = split.shift(),  // The next value is the first part of the site's path OR the whole part OR nothing (if the site has no path)
    path = optPath ? `/${optPath} : '', // add slash to path if it exists, empty otherwise
