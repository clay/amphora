...
function getSites() {
  ...
  files.getFolders(instanceSitesFolder).forEach(function (site) {
    ...
    if (!siteConfig) { // If no config.yaml file is found, we need to exit
      log('fatal', new Error(`Site "${site}" has directory but no config.yaml file`));
      process.exit(1);
    }
...
/**
 * [getSiteBySlug description]
 * @param  {[type]} uri [description]
 * @return {[type]}     [description]
 */
function getSiteBySlug(uri) {
  const slug = uri.split('/').shift();
