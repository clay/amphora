...
describe(_.startCase(filename), function () {
  ...
  describe('getTemplate', function () {
    ...
    it('returns template from node_modules with missing package.json', function () {
      var reference = 'domain.com/path/components/whatever',
        template = 'template.nunjucks';
    ...
    it('returns template with found package.json but missing template config', function () {
      var reference = 'domain.com/path/components/whatever',
        template = 'template.nunjucks';
      files.getComponentPackage.returns({});
      files.getComponentPath.returns('asdf/node_modules/asdf');
      sandbox.stub(glob, 'sync').returns([template]);
    ...
    it('returns template with found package.json and template config', function () {
      var reference = 'domain.com/path/components/whatever',
        goodTemplate = 'good.nunjucks',
        componentPath = 'asdf/node_modules/asdf',
        badTemplate = 'template.nunjucks';
      files.getComponentPackage.returns({template: goodTemplate});
      files.getComponentPath.returns(componentPath);
      sandbox.stub(glob, 'sync').returns([badTemplate]);

      expect(fn(reference)).to.eql(path.join(componentPath, goodTemplate));
    });
  });
