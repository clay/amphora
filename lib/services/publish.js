...
function resolvePublishUrl(uri, locals, site) {
  return function (pageData) {
    ...
    if (useResolvePublishUrl) {
      ...
      publishingChain = publishingChain.concat(site.resolvePublishUrl); // Run through the old API
    ...
    if ( publishingChain.length > 0 ) {
      ...
      return chain(publishingChain, uri, _.cloneDeep(pageData), locals)
        .then(function (pageUrl) {
          return publishPageAtUrl(pageUrl, uri, pageData, locals, site);
        })
        .catch(e => {
          throw e;
        });
