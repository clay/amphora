...
function mapSearchBatchOps(ops) {
 var searchOps = _.cloneDeep(ops);
  return _.filter(searchOps, function(op){
    var fullIndex = search.getSearchIndex(op.key);
...
function del(uri) {
  return get(uri).then(function (oldData) {
    ...
    if (componentModule && _.isFunction(componentModule.del)) {
      ...
      promise = db.del(uri).then(function(){
        return search.applyDelOrPut(uri, function(index, type){
          search.delete(index, type, uri);
        });
      });
    }
