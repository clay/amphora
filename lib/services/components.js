 ...
 * @returns {Array}
 */
function putDefaultBehavior(uri, data, locals) {
  const split = uri.split('@'),
    path = split[0],
    version = split[1],
    stringifiedData = locals && locals.site ? references.refPrefixToSlug(JSON.stringify(data), site, true) : JSON.stringify(data);

  if (version) {
    if (version === 'published') {
      return putPublished(path, stringifiedData);
    } else {
      return putTag(path, stringifiedData, version);
    }
  } else {
    return putLatest(path, stringifiedData);
  }
}
...
function put(uri, data, locals) {
  ...
  if (componentModule && (_.isFunction(componentModule.save) && callComponentHooks)) {
    ...
    result = putDefaultBehavior(uri, data, siteDefined || locals);
  }
...
function cascadingPut(uri, data, locals) {
  let dbUri = references.uriSwapInSlug(uri, locals.site); // prefix with slug
