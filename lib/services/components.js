...
function mapSearchBatchOps(ops) {
  return _.filter(ops, function(op){
    const componentType = references.extractComponentType(op.key);
    const version = references.extractVersion(op.key);
    op.indexType = (version === 'published') ? 'published' : 'draft';
    if (componentType){
      op.index = componentType;
    }
...
function cascadingPut(uri, data) {
  ...
  return getPutOperations(uri, data)
    .then(function (ops) {
      ...
      return search.batchDBFormat(mapSearchBatchOps(ops)).return(ops);
     }).then(function (ops) {
...
module.exports.mapSearchBatchOps = mapSearchBatchOps;
