...
function get(uri, locals) {
  ...
  console.log('components service get', uri, locals, name, componentModule);

  if (_.isFunction(componentModule)) {
    promise = componentModule(uri, locals);
  } else {
    promise = db.get(uri).tap(function (result) { console.log('db.get result', typeof result, result); }).then(JSON.parse);
  }
...
function cascadingPut(uri, data, locals) {
  console.log('components service cascadingPut', uri, data, locals);
...
function publish(uri, data, locals) {
  console.log('components service publish', uri, data, locals);
  if (data && _.size(data) > 0) {
    return cascadingPut(uri, data, locals);
  } else {
    return get(references.replaceVersion(uri), locals).then(function (data) {
      return composer.resolveComponentReferences(data, locals);
    }).then(function (composedData) {
      return cascadingPut(uri, composedData, locals);
    });
  }
}
 ...
 *
 * @param {string} name
 * @param {string} [templateName]  'template', 'sitemap', 'apple', 'facebook'
 ...
 * @param {string} templateName  i.e., 'template', 'sitemap', 'apple', 'facebook'
