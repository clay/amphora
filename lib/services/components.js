...
function getSchema(uri) {
  return bluebird.try(function () {
    var componentName = getName(uri),
      possSchema = schema.getSchema(files.getComponentPath(componentName));

    console.log(possSchema, typeof possSchema)
    if (possSchema) {
      return possSchema;
    } else {
      throw new Error('No schema found for ' + componentName);
    }
  });
}
