...
function get(uri, locals, swapSitePrefix = true) {
  let promise,
    dbUri = swapSitePrefix ? references.uriSwapInSlug(uri, locals.site) : uri, // We need to modify the uri to what the DB expects
...
function put(uri, data, locals) {
  let result,
    ...
    dbUri = siteDefined ? references.uriSwapInSlug(uri, siteDefined) : uri, // If locals isn't defined, we're bootstrapping and the key is correct
...
function cascadingPut(uri, data, locals) {
  const dbUri = references.uriSwapInSlug(uri, locals.site); // prefix with slug
...
function del(uri, locals) {
  const dbUri = references.uriSwapInSlug(uri, locals.site);
