 ...
 * @param {String} uri
 * @param {Object} [locals]
 * @param {Boolean} swapSitePrefix
 * @returns {Promise}
 */
function get(uri, locals, swapSitePrefix = true) {
  let promise,
    dbUri = swapSitePrefix ? clayUtils.uriPrefixToSlug(uri, locals.site) : uri, // We need to modify the uri to what the DB expects
    name = clayUtils.getComponentName(uri),
    componentModule = name && files.getComponentModule(name),
    callComponentHooks = _.get(locals, 'componenthooks') !== 'false',
    skipOrSwap = swapSitePrefix ? references.refSlugToPrefix(locals.site, true) : skip;
  ...
  if (componentModule && _.isFunction(componentModule.render) && callComponentHooks) {
    ...
    promise = bluebird.try(function () {
      return db.get(dbUri)
        .then(skipOrSwap) // Let's make sure we wap reference prefixes
        .then(JSON.parse) // Parse the JSON
        .then(upgrade.init(dbUri, locals)) // Run an upgrade!
        .then(function (data) {
...
function putLatest(uri, data) {
  return [
    { type: 'put', key: clayUtils.replaceVersion(uri), value: data }
  ];
}
...
function putPublished(uri, data) {
  return [
    { type: 'put', key: clayUtils.replaceVersion(uri, 'published'), value: data }
  ];
}
...
function putTag(uri, data, tag) {
  return [
    { type: 'put', key: clayUtils.replaceVersion(uri, tag), value: data }
  ];
}
...
function put(uri, data, locals) {
  let result,
    siteDefined = locals && locals.site,
    dbUri = siteDefined ? clayUtils.uriPrefixToSlug(uri, siteDefined) : uri, // If locals isn't defined, we're bootstrapping and the key is correct
    componentModule = files.getComponentModule(clayUtils.getComponentName(uri)),
    callComponentHooks = _.get(locals, 'componenthooks') !== 'false';

  // check for model.save() (plus no componenthooks flag)
  if (componentModule && (_.isFunction(componentModule.save) && callComponentHooks)) {
    ...
    result = bluebird.try(function () {
      ...
      return bluebird.resolve(componentModule.save(uri, data, locals)).then(function (resolvedData) {
        ...
        return {
          key: dbUri,
          type: 'put',
          value: siteDefined ? references.refPrefixToSlug(JSON.stringify(resolvedData), siteDefined, true) : JSON.stringify(resolvedData)
        };
      });
    }).tap(function () {
      const ms = timer.getMillisecondsSince(startTime);
    ...
    result = putDefaultBehavior(dbUri, siteDefined ? references.refPrefixToSlug(JSON.stringify(data), siteDefined, true) : JSON.stringify(data));
  }
...
function cascadingPut(uri, data, locals) {
  const dbUri = clayUtils.uriPrefixToSlug(uri, locals.site); // prefix with slug

  // split data into pieces
  return module.exports.getPutOperations(dbUri, data, locals).then(function (ops) {
    ...
    return db.batch(ops).then(function () {
      ...
      return rootOp && JSON.parse(references.refSlugToPrefix(locals.site, true)(rootOp.value));
    });
  });
}
...
function publish(uri, data, locals) {
  ...
  return get(clayUtils.replaceVersion(uri), locals)
...
function del(uri, locals) {
  const dbUri = clayUtils.uriPrefixToSlug(uri, locals.site);

  return get(uri, locals).then(function (oldData) {
    let promise,
      componentModule = files.getComponentModule(clayUtils.getComponentName(uri));

    if (componentModule && _.isFunction(componentModule.delete)) {
      promise = componentModule.delete(uri, oldData, locals)
        .then(function () {
          return db.del(dbUri);
        });
    } else {
      promise = db.del(dbUri).return(uri);
    }
...
function getTemplate(name, templateName) {
  ...
  // if there are slashes in this name, they've given us a reference like /_components/name/instances/id
  if (name.indexOf('/') !== -1) {
    name = clayUtils.getComponentName(name);
  }
...
function getIndices(ref, data) {
  ...
  components = _.uniq(_.filter(_.map(Object.keys(refs), function (ref) { return clayUtils.getComponentName(ref); }), _.identity));
