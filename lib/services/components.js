...
function get(uri, locals) {
  ...
  if (componentModule && _.isFunction(componentModule.render) && callComponentHooks) {
    ...
    promise = bluebird.try(function () {
      return db.get(dbUri)
        .then(references.swapOutAllUrisJson(locals.site)) // Let's make sure we wap reference prefixes
...
function putDefaultBehavior(uri, data, locals) {
  const split = uri.split('@'),
    path = split[0],
    version = split[1],
    stringifiedData = locals && locals.site ? references.swapInAllUrisJson(JSON.stringify(data), site) : JSON.stringify(data);
    ...
    result = bluebird.try(function () {
      ...
      return bluebird.resolve(componentModule.save(uri, data, locals)).then(function (resolvedData) {
        ...
        return {
          key: dbUri,
          type: 'put',
          value: siteDefined ? references.swapInAllUrisJson(JSON.stringify(resolvedData), siteDefined) : JSON.stringify(resolvedData)
        };
      });
    }).tap(function () {
      const ms = timer.getMillisecondsSince(startTime);
