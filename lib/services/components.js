 ...
 * @returns {Promise}
 */
function get(uri, locals, queryResults) {
  let promise,
  ...
  promise = db.get(uri)
    .then(JSON.parse)
    .then(upgrade.init(uri, locals));

  if (componentModule && callComponentHooks) {

    // fill in query
    if (_.isFunction(componentModule.query)) {
      promise = promise.then(data => queryResults ?
        Object.assign(data, {
          _queryResults: queryResults.results,
          _queryError: queryResults.error
        }) :
        scopeUtil
          .addQueryResults(uri, data, locals, componentModule)
          .toPromise(bluebird)
      );
    }
    ...
    if (_.isFunction(componentModule.render)) {
      ...
      promise = promise
        .then(data => componentModule.render(uri, data, locals))
        .tap(data => {
          if (!_.isObject(data)) {
            throw new Error('Component module must return object, not ' + typeof data + ': ' + uri);
          }
        })
        .then(data => _.omit(data, ['_queryResults', '_queryError']))
        .tap(() => {
...
function getIndices(ref, data) {
  ...
  components = _.uniq(_.filter(_.map(Object.keys(refs), function (ref) { return getComponentName(ref); }), _.identity));
