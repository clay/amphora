...
function get(uri, locals) {
  const name = getComponentName(uri),
    ...
    model = name && amphoraFs.getComponentModule(name),
    callComponentHooks = _.get(locals, 'hooks') !== 'false',
    executeRender = model && _.isFunction(model.render) && callComponentHooks,
    renderModel = reqExtension && amphoraFs.getComponentModule(name, reqExtension);
...
function put(uri, data, locals) {
  let model = amphoraFs.getComponentModule(getComponentName(uri)),
...
function del(uri, locals) {
  return get(uri).then(oldData => {
    let promise,
      componentModule = amphoraFs.getComponentModule(getComponentName(uri));
