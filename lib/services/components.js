...
function get(uri, locals) {
  let promise,
    dbUri = references.uriSwapInSlug(uri, locals.site), // We need to modify the uri to what the DB expects
  ...
  if (componentModule && _.isFunction(componentModule.render) && callComponentHooks) {
    ...
    promise = bluebird.try(function () {
      return db.get(dbUri)
        .then(references.refSlugToPrefix(locals.site, true)) // Let's make sure we wap reference prefixes
